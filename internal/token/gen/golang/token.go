...
func GenToken(pkg, outdir string, tokMap *token.TokenMap) {
	// write to parent dir
	tokenPath := path.Join(path.Dir(outdir), "token", "token.go")
	tmpl, err := template.New("token").Parse(TokenMapSrc[1:])
	if err != nil {
		panic(err)
	}
	buf := new(bytes.Buffer)
	err = tmpl.Execute(buf, nil) //TokenData{TypMap: tokMap.TypeMap, IdMap: typeMap(tokMap)})
	if err != nil {
		println(err.Error())
	}
	...
	// write parser.TokMap
	mapFile := path.Join(outdir, "parser", "tokmap.go")
	tmpl2, err := template.New("tokmap").Parse(TokenMapValue[1:])
	if err != nil {
		panic(err)
	}
	buf2 := new(bytes.Buffer)
	err = tmpl2.Execute(buf2, TokenData{Pkg: path.Join(path.Dir(pkg), "token"), TypMap: tokMap.TypeMap, IdMap: typeMap(tokMap)})
	if err != nil {
		println(err.Error())
	}

	// Use go/format to indent the idMap literal correctly.
	source2, err := format.Source(buf2.Bytes())
	if err != nil {
		println(err.Error())
		// panic(err)
	}
	io.WriteFile(mapFile, source2)
}
...
// NOTE: write to parser dir
const TokenMapValue string = `
// Code generated by gocc; DO NOT EDIT.
